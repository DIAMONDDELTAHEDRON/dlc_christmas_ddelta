-- More optimized version of GMPath
...
function GMPath:addInternalPoint(x, y, speed)
    if #self.intpoints > 0 then
        local last_point = self.intpoints[#self.intpoints]
        self.total_length = self.total_length + MathUtils.dist(last_point.x, last_point.y, x, y)
    end
...
function GMPath:getTotalLength(recalculate)
    if recalculate then
        self.total_length = 0
        for i = 1, #self.intpoints - 1 do
            local point_a = self.intpoints[i]
            local point_b = self.intpoints[i + 1]
            self.total_length = self.total_length + MathUtils.dist(point_a.x, point_a.y, point_b.x, point_b.y)
        end
...
function GMPath:getPointAtLength(length)
    local total_length = self:getTotalLength()

    if length <= 0 then
        return self.intpoints[1]
    elseif length >= total_length then
        return self.intpoints[#self.intpoints]
    end
    ...
    for i = 1, #self.intpoints - 1 do
        local point_a = self.intpoints[i]
        local point_b = self.intpoints[i + 1]
        local segment_length = MathUtils.dist(point_a.x, point_a.y, point_b.x, point_b.y)

        if accumulated_length + segment_length >= length then
            local remaining_length = length - accumulated_length
            local t = remaining_length / segment_length
            return self:createPoint(
